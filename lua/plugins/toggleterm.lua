return {
	{
		...
		config = function()
			require("toggleterm").setup({
				...
				size = 50, -- Default size for horizontal/vertical if not overridden by specific mapping
				...
				float_opts = {
					border = "curved",
					width = 150, -- Default width for floating terminals
					height = 30, -- Default height for floating terminals
				},
			...
			vim.keymap.set(
				...
				{ desc = "Toggle Default Floating Terminal" }
			)
			vim.keymap.set(
				"t",
				"<leader>t",
			...
			vim.keymap.set(
				...
				{ desc = "Toggle Vertical Split Term (70% width)" }
			)
			vim.keymap.set(
				"t",
				"<leader>gv",
			...
			vim.keymap.set(
				...
				{ desc = "Toggle Horizontal Split Term (80% height)" }
			)
			vim.keymap.set(
				"t",
				"<leader>gh",
